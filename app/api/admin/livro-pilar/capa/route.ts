...
import { LIVRO_PILAR, PILAR_CAPA_ESTILOS, PILAR_CAPA_ESTILO_DEFAULT, PILAR_CAPA_PALETA } from '@/lib/livro-pilar';

export const runtime = 'nodejs';
export const maxDuration = 300;

// POST { estilo? } — gera UMA variante de capa do pilar (Replicate flux-1.1-pro,
...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => ({}))) as { estilo?: string };
  const estilo = PILAR_CAPA_ESTILOS[body.estilo ?? ''] ?? PILAR_CAPA_ESTILOS[PILAR_CAPA_ESTILO_DEFAULT];
  const prompt = `${LIVRO_PILAR.cena}\n\n${estilo.prompt}\n\n${PILAR_CAPA_PALETA}\n\n${SAFETY}`;
  ...
  let imageUrl = out;
  try {
    imageUrl = await guardarImagem(out, `livro-pilar/${LIVRO_PILAR.slug}/capa-${ts}.jpg`);
  } catch {
