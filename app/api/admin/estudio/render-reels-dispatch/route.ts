...
// POST { dias?: "2" | "2,5,9" } — dispara workflow render-reels (TTS + FFmpeg)
...
export async function POST(req: Request) {
  ...
  const jobId = `reels-${dias ? `d${dias.replace(/,/g, '-')}-` : ''}${Date.now()}`;
  ...
  const res = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/actions/workflows/render-reels.yml/dispatches`,
    {
      method: 'POST',
      headers: {
  ...
  return NextResponse.json({
    ...
    siteUrl,
    dias,
    workflowRunUrl: `https://github.com/${owner}/${repo}/actions/workflows/render-reels.yml`,
  });
}
