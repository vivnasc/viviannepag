...
// SOULAB · (re)gera a imagem (Flux) de UMA peça. Usa o prompt guardado
// (notaVisual) ou um novo enviado. Não reescreve o texto. Limpa o vídeo (a peça
// volta a precisar de render com a imagem nova).
async function fundoImagem(prompt: string, slug: string): Promise<{ url: string | null; erro?: string }> {
  ...
  for (let t = 0; t < 4; t++) {
    try {
      ...
      try { return { url: await guardarImagem(url, `soulab/${slug}/fundo-${Date.now()}.jpg`) }; } catch { return { url }; }
    } catch (e) {
...
export async function POST(req: Request) {
  ...
  if (dias[0]) dias[0].videoUrl = null; // imagem nova => render fica por refazer
