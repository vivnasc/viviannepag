              ...
              return (
                <div key={p.slug} className="rounded-xl border overflow-hidden" style={{ background: 'rgba(255,255,255,0.02)', borderColor: sel.has(p.slug) ? DZ : 'rgba(255,255,255,0.1)' }}>
                  ...
                  <div className="p-2 flex flex-wrap gap-1 text-[0.62rem]">
                    ...
                    <button onClick={() => acao(p.slug, '/api/admin/crescer/imagem', { mundo: true, imagemModo: 'cena' }, 'A gerar nova imagem pelo MUNDO (cena da civilização)…', 'Imagem nova do mundo. (atualizada com o motor novo, sem mexer no texto.)', false)} disabled={tBusy} title="gera uma imagem NOVA pelo motor actual (documentário da civilização), substituindo a antiga — sem tocar no texto" className="px-2 py-1 rounded border disabled:opacity-40" style={{ borderColor: DZ, color: DZ }}>🌍 nova imagem</button>
