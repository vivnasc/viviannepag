...
async function main() {
  ...
  for (const d of dias) {
    ...
    if (kinetic) {
      ...
      let DUR = duasFaces ? 17 : nbeats ? Math.max(16, Math.round(4.8 * slides.length)) : carta ? Math.max(16, Math.round(4.6 * slides.length)) : seqPorMomento ? Math.max(8, Math.round(soulabSeg * slides.length)) : visual ? 9 : 8;
