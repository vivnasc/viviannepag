...
${carta ? `A PERSONAGEM desta carta (é só sobre ela, não sobre o véu): ${personagem.nome}. ${personagem.essencia} Diz coisas como: ${personagem.frases.map((x) => `"${x}"`).join('; ')}. A sombra: ${personagem.sombra}. Tira daqui 3 a 4 comportamentos CONCRETOS e do dia a dia para a anáfora, e fecha em "Sou aquela.".` : naoNorm ? `AS LINHAS DE HOJE: cada beat é uma frase "Não normalizes…" ORIGINAL, escrita AGORA, com um detalhe CONCRETO e específico de 2026 (um objeto, uma app, uma cena real do dia a dia). REGRA DURA: é PROIBIDO usar, copiar ou aproximar-te das frases-exemplo abaixo — servem só para veres o TIPO de assimetria; se repetires as palavras delas, FALHASTE. Cada beat uma assimetria DIFERENTE; não juntes várias numa linha.
