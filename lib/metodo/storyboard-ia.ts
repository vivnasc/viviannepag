...
- O TIPO de assimetria (fica nestas duas famílias, mas inventa o exemplo): (1) responsabilidade invisível sem autoridade equivalente — carregar a gestão de tudo sem mandar em nada; (2) gestão emocional — ser responsável pelo estado emocional de todos menos pelo seu.
- EXEMPLOS só de TOM (NÃO copiar, NÃO reutilizar estas palavras): ${(c.bancoNaoNormalizes ?? []).slice(0, 2).map((x) => `"${x}"`).join(' · ')}
