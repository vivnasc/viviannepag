...
- O TEMA NÃO é "fazer muito" nem tarefas domésticas isoladas (lavar pratos). É a ASSIMETRIA invisível: (1) RESPONSABILIDADE INVISÍVEL SEM AUTORIDADE EQUIVALENTE e (2) GESTÃO EMOCIONAL. A faca está no DESEQUILÍBRIO, não no esforço.
- Cada beat é uma frase que começa por "Não normalizes" + UMA assimetria concreta e observável (quem é responsável vs quem só "ajuda"; a lista dela obrigatória vs a dele opcional; a paz da casa a depender do estado emocional dela).
...
Devolve SÓ JSON válido: {"beats":[{"tempo":"0-1s","imagem":"o que se vê (na veste, em movimento)","texto":"o que aparece no ecrã ou a voz-off"}, ...],"envio":"..."} com ${fmt.beats} beats.${carta ? ' O texto do ÚLTIMO beat é exatamente "Sou aquela." sozinho.' : ''}${naoNorm ? ' Cada beat (menos o último) começa por "Não normalizes" e o ÚLTIMO beat é exatamente "Espera. Isto não é normal." sozinho.' : ''}${cartaRen ? ' O 1.º beat é a CAPA que PARA O SCROLL: uma CENA concreta (fotografia/memória que se vê), nunca identidade nem tese. Os beats seguintes são o CORPO da carta (papel, as palavras a revelarem-se), na ordem da arquitetura: vida por trás, nome antigo, releitura (a viragem onde a carta acontece), preço, abertura.' : ''}`;
