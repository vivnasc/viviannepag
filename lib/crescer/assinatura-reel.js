...
// (soltar/ver/vir/viver são do método, separados). Nomes limpos, curtos.
const LABELS = {
...
function motivoSVG(id) {
  // FOCO de luz no centro (o "sol" da referencia), pequeno e brilhante, em vez de um halo
  // mole por tras (que tornava a geometria baca e grossa). A geometria fica fina e nitida.
  const glow = `<circle class="halo" cx="50" cy="50" r="7" fill="url(#cg)"/>`;
  const c = componente(id);
  if (c) return glow + c.inner;
  // fallback simples (nunca deve acontecer): 3 aneis concentricos.
  return glow + `<g fill="none" stroke="${TOK.gold}" stroke-width="0.7" stroke-linecap="round"><circle cx="50" cy="50" r="14" pathLength="1"/><circle cx="50" cy="50" r="26" pathLength="1"/><circle cx="50" cy="50" r="38" pathLength="1"/></g>`;
}
