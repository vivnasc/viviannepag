...
const RECEITAS_GEO = {
  consciencia: pRings, eumaior: pRings, raizes: pRings,
  sentido: pOrbit, ciclos: pOrbit, desencaixe: pOrbit,
  corpo: pContour,
  vinculos: pVesica, transformacao: pVesica,
  campo: pConst, emergencia: pConst,
  sombra: pEclipse,
};
// a COMPOSIÇÃO da peça (varia por seed dentro do tema; sempre a mesma linguagem).
function geometriaVDS(tema, seed) {
  const fn = RECEITAS_GEO[tema] || pRings;
  return fn(hashStr(String(seed || tema || '')));
}
