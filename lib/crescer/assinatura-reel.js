...
function reelHTML(opts) {
  ...
  const linhas = (Array.isArray(o.linhas) ? o.linhas : [o.capa || o.frase]).filter(Boolean).slice(0, 4);
  const N = Math.max(1, linhas.length);
  const cyc = Math.max(8, N * 4 + 2);
  const marca = o.marca || 'vivianne dos santos';
  const assinatura = o.assinatura || 'a vantagem de crescer';

  let kf = '', beats = '';
  linhas.forEach((t, i) => {
    const s = (i * 4) / cyc * 100, inp = ((i * 4) + 0.6) / cyc * 100, os = ((i * 4) + 3.4) / cyc * 100, op = ((i * 4) + 4) / cyc * 100;
    kf += `@keyframes beat${i}{0%,${s.toFixed(1)}%{opacity:0;transform:translateY(10px)}${inp.toFixed(1)}%{opacity:1;transform:none}${os.toFixed(1)}%{opacity:1}${op.toFixed(1)}%{opacity:0;transform:translateY(-8px)}100%{opacity:0}}\n`;
...
<body><div class="reel">
  ...
  <svg class="geo" viewBox="0 0 100 100">
    ...
    ${motivoSVG(rec.motivo)}
  </svg>
  <div class="top"><span>a mãe</span><span>${esc(label)}</span></div>
...
module.exports = { reelHTML, RECEITAS, receitaDe, motivoSVG, TOK };
