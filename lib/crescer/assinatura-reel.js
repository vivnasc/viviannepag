...
function slideHTML(opts) {
  ...
  // geometria varia POR POST (seed) e por TELA (idx): 2 posts / 2 telas nunca iguais.
  const seqM = motivosDoTema(tema);
  const motivo = seqM[(hashStr(o.seed || texto) + idx - 1) % seqM.length];
  // layout ajustavel (autonomia): posicao/tamanho do texto e da geometria.
  const geoTop = o.geoTop != null ? o.geoTop : 14, geoW = o.geoW != null ? o.geoW : 56;
  const txtY = o.txtY != null ? o.txtY : 20, txtSize = o.txtSize != null ? o.txtSize : 5.4;
  return `<!doctype html><html lang="pt"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<style>
  ...
  *{ box-sizing:border-box; margin:0; padding:0; }
  ...
  .disc{ fill:var(--bg); stroke:var(--gold); stroke-width:1.1; }
  .copy{ position:absolute; left:9%; right:9%; bottom:${txtY}%; text-align:center; }
